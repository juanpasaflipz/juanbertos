...
import { BurritoNotASnackContent } from "./BurritoNotASnackContent";

const SITE_URL = "https://www.juanbertos.com";
const IMAGE_PATH = "/menu/california-burrito.jpg";
const SLUG = "why-a-california-burrito-isnt-a-snack";

type Props = { params: Promise<{ locale: string }> };

export async function generateMetadata({ params }: Props) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "burritoNotASnack.meta" });
...
export default async function BurritoNotASnackPage({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "burritoNotASnack" });
